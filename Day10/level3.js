...
function mostSpokenLanguages(countries, num) {
  const countriesLanList = [];
  countries.forEach((value) => {
    value.languages.forEach((lan) => {
      countriesLanList.push(lan);
    });
  });
  const lanSet = new Set(countriesLanList);
  let numOfLan = lanSet.size;
  const count = [];
  for (const l of lanSet) {
    const filLans = countriesLanList.filter((lng) => lng == l);
    //console.log(filLans);
    count.push({ Lang: l, Count: filLans.length });
  }
  count.sort((a, b) => {
